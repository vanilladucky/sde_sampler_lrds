...
from sde_sampler.distr.gauss import GMM, ManyModes
...
# Run the big loop
dump_results = []
for dim, n_modes, mixture_weight_factor, var in tqdm(
        list(itertools.product(dim_range, n_modes_range, mixture_weight_factor_range, var_range))):
    # Build the target
    target = ManyModes(dim=dim, n_modes=n_modes, mixture_weight_factor=mixture_weight_factor, var=var)
    target.to(device)
    # Build a dataset
    ...
    weights_ref, means_ref, variances_ref = fit_gmm(
        n_components=target.loc.shape[0],
        dataset=dataset,
    ...
    model = make_model(
        ...
        target_details=make_target_details(target_name='many_modes', dim=dim, n_modes=n_modes,
                                           mixture_weight_factor=mixture_weight_factor, var=var),
        training_details={
            'train_steps': args.train_steps,
            'train_batch_size': args.train_batch_size,
            'eval_batch_size': args.eval_batch_size
        },
        force_vp20=args.solver_type == 'vp-ref')
    # Train the model
    ...
    results_weight_hists = [model.target.compute_mode_count(final_results.samples).cpu()]
    for _ in range(args.n_sampling_seeds - 1):
        ...
        results_weight_hists.append(model.target.compute_mode_count(final_results_new.samples).cpu())
